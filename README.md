...
- **Generate ideas** grounded in each paper's constraints, assumptions, and
  inductive biases. Each idea comes with suggested search queries for
...
# Install with local LLM support (recommended):
pip install -e ".[dev,llm]"
# The [llm] extra installs llama-cpp-python, which needs CMake + a C++
# compiler. On Windows, install Visual Studio Build Tools first.
#
# Without [llm], everything works except local LLM generation
# (summaries, ideas, novelty checks). You can still search, import,
# enrich, and query the library.
```
...
```bash
cp .env.example .env
# Edit .env and add your HF_TOKEN (used only for downloading models
# from Hugging Face Hub — no remote inference is performed).
```
...
This downloads:
- **BGE embedding + reranker models** (cached by sentence-transformers).
- **Qwen2.5-1.5B-Instruct GGUF** (Q4_K_M quantization, ~1 GB) for local LLM.
...
**MCP server** (for coding agents like Claude Code, Cursor, Freebuff):

```bash
python -m src.mcp_server
# Communicates over stdio by default. Set MCP_TRANSPORT=sse in .env
# for SSE mode.
```
...
```bash
# Unit + component + integration tests (mocked, offline, ~7s)
pytest

# Smoke tests (gitignored — require real models + network)
# Run after downloading models:
...
the user's machine. Cloud / HPC scaling is future work.
